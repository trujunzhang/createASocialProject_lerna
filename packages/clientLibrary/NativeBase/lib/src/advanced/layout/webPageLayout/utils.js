...
export function jsonToQueryString(json) {
    const items = Object.keys(json).map((key) => encodeURIComponent(key) + '=' + encodeURIComponent(json[key]));
    if (items.length) {
        return `?${items.join('&')}`;
    }
    return '';
 ...
 * @param url
 */
export function isTwitterCallBack(url) {
    return url.indexOf('oauth_token') !== -1 && url.indexOf('//politicl.com') !== -1;
}
export function isFacebookCallBack(url) {
 ...
 * @param url
 */
export function isShareCallBack(url) {
