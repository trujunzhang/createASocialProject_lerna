...
import { Dimensions, View, Animated, ScrollView, StyleSheet } from 'react-primitives';
// TODO:DJZHANG(NB)
const TimerMixin = null;
...
const ScrollableTabView = createReactClass({
    ...
    renderScrollableContent() {
        ...
        return (React.createElement(ScrollView, Object.assign({ horizontal: true, pagingEnabled: true, automaticallyAdjustContentInsets: false, keyboardShouldPersistTaps: "handled", contentOffset: {
            ...
            }, onMomentumScrollBegin: this._onMomentumScrollBeginAndEnd, onMomentumScrollEnd: this._onMomentumScrollBeginAndEnd, scrollEventThrottle: 16, scrollsToTop: false, showsHorizontalScrollIndicator: false, scrollEnabled: !this.props.locked, directionalLockEnabled: true, alwaysBounceVertical: false, keyboardDismissMode: "on-drag" }, this.props.contentProps), scenes));
    },
    _composeScenes() {
        return this._children().map((child, idx) => {
            let key = this._makeSceneKey(child, idx);
            const RNView = View;
            return (React.createElement(SceneComponent, { key: child.key, shouldUpdated: this._shouldRenderSceneKey(idx, this.state.currentPage), style: { width: this.state.containerWidth } }, this._keyExists(this.state.sceneKeys, key) ? (child) : (React.createElement(RNView, { heading: child.props.heading }))));
        });
    },
...
export const Tabs = ScrollableTabView;
